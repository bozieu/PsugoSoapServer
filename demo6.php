...
if(isset($_GET['name'])){
	...
	require_once('php2wsdl/class.phpwsdlclient.php');// All depencies are loaded here
	ini_set('soap.wsdl_cache_enabled',0);	// Disable caching in PHP
	PhpWsdl::$CacheTime=0;					// Disable caching in PhpWsdl
	$client=new PhpWsdlClient('http://localhost/PsugoSoapServer/php2wsdl/demo4?WSDL');// The constructor has to be called with the target webservice WSDL URI
	?><html><head><title>SOAP demo</title></head><body><p><?
	echo htmlentities($client->Hello($_GET['name']));// Perform the SOAP request and output the response
